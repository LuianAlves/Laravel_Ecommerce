            ...
            <div class="row">
                ...
                <div class="col-xs-12 col-sm-12 col-md-9 homebanner-holder">
                    ...
                    <section class="section latest-blog outer-bottom-vs wow fadeInUp">
                        <h3 class="section-title">{{ session()->get('language') == 'portuguese' ? 'Últimas Postagens do Blog' : 'latest form blog' }}</h3>
                        <div class="blog-slider-container outer-top-xs">
                            <div class="owl-carousel blog-slider custom-carousel">
                                @php   
                                    $blog_post = App\Models\Blog\BlogPost::orderBy('id', 'DESC')->get(); 
                                @endphp
                                    ...
                                    <div class="item">
                                        <div class="blog-post">
                                            <div class="blog-post-image">
                                                <div class="image"> 
                                                    <a href="{{ route('blog.details', $post->id) }}"><img src="{{ asset($post->post_image) }}"></a>
                                                </div>
                                            </div>

                                            <div class="blog-post-info text-left">
                                                <h3 class="name">
                                                    <a href="{{ route('blog.details', $post->id) }}">{{ session()->get('language') == 'portuguese' ? strtoupper($post->post_title_pt) : strtoupper($post->post_title_en) }}</a>
                                                </h3>
                                                <span class="info">{{ Carbon\Carbon::parse($post->created_at)->diffForHumans() }}</span>
                                                <p class="text">
                                                    {{ session()->get('language') == 'portuguese' ? Str::limit($post->post_details_pt, 80) : Str::limit($post->post_details_en, 80) }}
                                                </p>
                                                <a href="{{ route('blog.details', $post->id) }}" class="lnk btn btn-primary">{{ session()->get('language') == 'portuguese' ? 'Leia mais ..' : 'Read more ..' }}</a>
                                            </div>
